...
# ================= FastAPI 实例初始化 =================
app = FastAPI(title="B站多维舆情获取服务", description="为 Agent 提供基础信息、弹幕与评论的结构化数据。")
...
async def get_video_meta(req: BasicRequest):
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"获取视频基础信息失败: {e}")
...
async def get_video_danmaku(req: DanmakuRequest):
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"获取弹幕失败: {e}")
